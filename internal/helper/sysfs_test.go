...
func TestSystemDisksPlainRoot(t *testing.T) {
	...
	got, err := systemDisksFrom(mounts, sys.devBlock())
	if err != nil {
		t.Fatal(err)
	}
...
func TestSystemDisksFailClosed(t *testing.T) {
	...
	if _, err := systemDisksFrom([]mount{{majMin: "0:40", mountpoint: "/"}}, sys.devBlock()); err == nil {
		t.Fatal("expected error for a root that is not a block device")
	}
	if _, err := systemDisksFrom(nil, sys.devBlock()); err == nil {
